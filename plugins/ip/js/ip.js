					...
					success: function(data) {
						if (data.eleDetail.length > 0) {
							...
							for(var k = 0 ; k <data.eleDetail.length ; k++ ){
								...
								appendHtml = appendHtml + '<li data-key="'+info.chr_id+'@'+encodeURI(info.chr_name, "utf-8")+'@'+info.chr_code+'" class="bdsug-store bdsug-overflow" onmousedown="ip._eleLiClick(\''+id+'\',this)">'+info.codename+'</li>'
							}
						...
						if(text.split(",").length-1 > 1){
							...
							for(var j =0; j <= keyWord.length+1; j++){
								...
								if(j > keyWord.length){
									...
									if(pattern.test(text)){
										var newHtml = text.replace(pattern, "<font color='"+keyColor+"' style='background: yellow' class='ipfont'>" + keyWord + "</font>");
										var p = '<p style="text-align: right !important;">'+newHtml+'</p>';
										nodes.eq(i).html(p);
										break;
									}
									...
									var pattern = new RegExp( keyWord.insert(j,','), "gi");
									if(pattern.test(text)){
										var newHtml = text.replace(pattern, "<font color='"+keyColor+"' style='background: yellow' class='ipfont'>" + keyWord.insert(j,',') + "</font>");
										var p = '<p style="text-align: right !important;">'+newHtml+'</p>';
										nodes.eq(i).html(p);
										break;
									}
...
ip.fuzzyQuery = function(data,id,viewModel,gridArea) {
	...
	if(search_text.indexOf(".") != -1){
		var lastTwoWord = search_text.split('.')[1];
		if(lastTwoWord != ""){
			if(lastTwoWord.charAt(1) == "0" && lastTwoWord.charAt(0) == "0"){
				search_text = search_text.split('.')[0];
			}else if(lastTwoWord.charAt(1) == "0" && lastTwoWord.charAt(0) != "0"){
				search_text = search_text.split('.')[0] +"."+lastTwoWord.charAt(0);
			}
		}else{
			search_text = search_text.split('.')[0];
			searchVal = search_text.split('.')[0];
		}
	}
	var result = [];
	if (search_text == "") {
...
//	result = result.slice(0,49);
	dataTable.setSimpleData(result, {
		unSelect: true
	},1,totnum,search_text);//youbha 瀑布式模糊查询search_text
	if(pagenum === 0) {
		pagenum = 1;
	}
...
ip.showBigValue = function(id,type,num) {
	$('body').on('focus',"#" + id,function(){
		...
		var value = $("#" + id).val();
		if(type == 'bankCard'){
			$("#" + id).val(value.replace(/[^\d\-]/g,'').replace(/(^[\-])/g,'').replace(/([\-][\-])/g,''));
			var newValue =  $("#" + id).val();
...
ip.initSumGrid = function(areaid,datas) {
	...
	viewModel.createGrid = function(data) {
		...
		innerHTML +="<div options='"+'{"field":"sumval","editType":"string","dataType":"String","title":"小计值","width":"100"}'+"'></div>";	
		innerHTML += "</div>";
	...
	for(var k = 0 ; k <name.length ;k++){
		if(name[k]){
			...
			if(gridObj.getSelectRows().length > 0){
				...
				op['sumval'] = ip.dealThousands(gridObj.dataSourceObj.getSumValue(filed,column,gridObj,true));
			}else{
				op['sumval'] = ip.dealThousands(0);
			}
